...
from applet.gui.common_config import wx_notify_token
...
class NotifyService(common_service.CommonService):
    def __init__(self, notify_email: str, base_output_path, file_list: [FileInfo], logger,
                 step=13,
    ...
    def deal_send_wx(self, run_info_list, pred_info_list, run_data_list):
        logger = self.logger
        try:
            if wx_notify_token is None or len(wx_notify_token) == 0:
            ...
            token_list = wx_notify_token.split(';')
    ...
    def build_wx_msg_markdown_content(self, run_info_list: [RunInfo], pred_info_list: [PredInfo],
        ...
        for run_info in run_info_list:
            ...
            run_id_qual_feat_list.append(
                '{}: {}'.format(run_info.run_id, ';'.join(run_id_qual_feat_dict.setdefault(run_info.run_id, []))))
            run_id_unqual_feat_list.append(
                '{}: {}'.format(run_info.run_id, ';'.join(run_id_unqual_feat_dict.setdefault(run_info.run_id, []))))

            f6_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '6'))))
            f7_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '7'))))
            f8_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '8'))))
            f10_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '10'))))
            f11_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '11'))))
            f13_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '13'))))
            f14_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '14'))))
            f17_data_list.append(
                '{}: {}'.format(run_info.run_id, run_data_seq_dict.get('{}_{}'.format(run_info.seq_id, '17'))))

        # markdown_content1 = '#### Run_id: \n\tR01U0001: WAK20240101gaohh_45minDIA.raw\n\tR01U0002: WAK20240102gaohh_45minDIA.raw'
        markdown_content1 = '#### Run_id: {}'.format('\n\t'.join(run_id_name_list))
        # markdown_content2 = '#### LC-MS performance: \n\tR01U0001: Qualified; Qualified\n\tR01U0002: Qualified; Qualified'
        markdown_content2 = '#### LC-MS performance: {}'.format('\n\t'.join(run_id_name_pred_list))

        markdown_content3 = '#### Qualified features: {}'.format('\n\t'.join(run_id_qual_feat_list))
        markdown_content4 = '#### Unqualified features: {}'.format('\n\t'.join(run_id_unqual_feat_list))
        markdown_content5 = '## Features description:'

        markdown_content6 = '#### F6 (Intensity valriation of adjacent MS1 scan): {}'.format('\n\t'.join(f6_data_list))
        markdown_content7 = '#### F7 (Median MS1 accuracy of identified precursors): {}'.format(
            '\n\t'.join(f7_data_list))
        markdown_content8 = '#### F8 (TIC MS1 signal): {}'.format('\n\t'.join(f8_data_list))
        markdown_content9 = '#### F10 (Median MS2 accuracy of identified precursors): {}'.format(
            '\n\t'.join(f10_data_list))
        markdown_content10 = '#### F11 (TIC MS2 signal): {}'.format('\n\t'.join(f11_data_list))
        markdown_content11 = '#### F13 (Number of identified peptides): {}'.format('\n\t'.join(f13_data_list))
        markdown_content12 = '#### F14 (Number of identified proteins): {}'.format('\n\t'.join(f14_data_list))
        markdown_content13 = '#### F17 (Ion mobility accuracy): {}'.format('\n\t'.join(f17_data_list))
